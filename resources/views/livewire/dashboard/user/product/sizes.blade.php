<div>
    ...
    <div class="modal fade" wire:ignore.self id="deleteSizeConfrmationModal" aria-hidden="true"
         aria-labelledby="..."
         tabindex="-1">
                ...
                <div class="modal-body text-center p-5">
                    ...
                    <div class="mt-4 pt-4">
                        ...
                        <div class="buttons_modal mt-3">
                            <button class="btn btn-primary" wire:click="deleteSizeItem">
                                {{__('msg.continue')}}
                            </button>
    ...
    <div class="modal fade" wire:ignore.self id="deleteSizesListConfrmationModal" aria-hidden="true"
         aria-labelledby="..."
         tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body text-center p-5">
                    ...
                    <div class="mt-4 pt-4">
                        ...
                        <!-- Toogle to second dialog -->
                        <div class="buttons_modal mt-3">
                            <button class="btn btn-primary" wire:click="deleteSizesList">
                                {{__('msg.continue')}}
                            </button>
                            <button type="button" class="btn btn-light"
                                    data-bs-dismiss="modal">{{__('msg.close')}}</button>
    ...
    <div class="modal fade" wire:ignore.self id="addSizeItemModal" aria-hidden="true" aria-labelledby="..."
         tabindex="-1">
        <div class="modal-dialog modal-dialog-centered  modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">{{__('msg.add_new_size')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body p-5">
                    <div class="card-body">
                        <form wire:submit.prevent="storeSizeItem">
    ...
    <div class="modal fade" wire:ignore.self id="editSizeItemModal" aria-hidden="true" aria-labelledby="..."
         tabindex="-1">
        <div class="modal-dialog modal-dialog-centered  modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">{{__('msg.edit_size')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body p-5">
                    <div class="card-body">
                        <form wire:submit.prevent="updateSizeItem">
                            <div class="live-preview">
                                <div class="row gy-4">
                                    <div class="col-xxl-3 col-md-6">
                                        <div>
    ...
    <div x-data="tableHandler()" x-init="init()" class="table-responsive">
        ...
        <div class="card-header border-bottom-dashed">
            ...
            <div class="row g-4 align-items-center">
                <div class="col-sm">
                    <div>
                        <h5 class="card-title mb-0">{{__('msg.sizes')}}</h5>
                    </div>
                </div>
                <div class="col-sm-auto">
                    <div>
                        <button class="btn btn-danger cursor-pointer"
                                wire:click="deleteSizesListConfirmation"
                                :disabled="!hasSelected">
                            <i class="ri-delete-bin-2-line"></i></button>
                        <button type="button" class="btn btn-success add-btn cursor-pointer"
                                wire:click="createSizeItem()">
                            <i class="ri-add-line align-bottom me-1"></i>{{__('msg.add_size')}}
                        </button>
                    </div>
                </div>
            </div>
                                    ...
                                    <th scope="row">
                                        <div class="form-check">
                                            <input
                                                ...
                                                type="checkbox"
                                                wire:model.defer="sizes_list"
                                                value="{{ $size->id }}"
                                                @change="updateHasSelected">
                                        </div>
                                    </th>
                                    <td class="text-center">{{ $size->name}}</td>
                                    <td class="text-center">{{ (float)$size->price}}</td>
                                    <td class="text-center">
                                        <div class="form-check form-switch text-center">
                                            <input
                                                class="form-check-input"
                                                wire:change="activeSizeItem({{ $size->id }})"
                                                type="checkbox"
                                                {{ $size->active == 1 ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <a wire:click="editSizeItem({{ $size->id }})"
                                           class="text-primary mr-2 cursor-pointer">
                                            <i class="ri-pencil-fill fs-20"></i>
                                        </a>
                                        <a wire:click="deleteSizeConfirmation({{ $size->id }})"
